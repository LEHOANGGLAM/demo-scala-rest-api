...
//
//  def createProduct: String = config.get[String]("external.products.createProduct")
//
//  override def listAll(): Future[Iterable[Product]] = client.get[Seq[Product]](getAllProducts)
//
//  override def save(product: Product): Future[Product] = client.post[Product](createProduct, Some(Json.toJson(product)))
////  implicit val productReads: Reads[Product] = Json.reads[Product]
////  implicit val seqProductReads: Reads[Seq[Product]] = Reads.seq(productReads)
//}
//
