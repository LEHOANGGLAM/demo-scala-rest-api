...
@Singleton
class ExternalProductClient @Inject()(ws: WSClient, config: Configuration)(
  implicit ec: ExecutionContext
) extends AbstractHttpClient(ws)
  with Logging {
  private def baseUrl: String =
    config.get[String]("external.products.auth.basic.url")

  private def username: String =
    config.get[String]("external.products.auth.basic.username")

  private def password: String =
    config.get[String]("external.products.auth.basic.password")

  override protected def provideAuth: (String, String, WSAuthScheme) =
    (username, password, WSAuthScheme.BASIC)

  override protected def provideRequestFilters: Seq[WSRequestFilter] =
    Seq(AhcCurlRequestLogger())
  ...
  def get[A <: Object](
                      ...
                      )(implicit reads: Reads[A]): Future[A] =
    handleResponseEntity(
      super.get(buildRequestURL(apiPath), params, extraHeaders)
    )
  ...
  def put[A <: Object](
    ...
    handleResponseEntity(
      super.put(buildRequestURL(apiPath), params, requestBody, extraHeaders)
    )
  ...
  def post[A <: Object](
    ...
    handleResponseEntity(
      super.post(buildRequestURL(apiPath), requestBody, params, extraHeaders)
    )
  ...
  def delete[A <: Object](
    ...
    handleResponseEntity(
      super.delete(buildRequestURL(apiPath), params, extraHeaders)
    )

  private def handleResponseEntity[A](
  ...
  private def handleError(exception: Throwable): Throwable = exception match {
    case exception: ExternalServiceException =>
      logger.error(
        s"ExternalServiceException statusCode: ${exception.statusCode} message: ${exception.message}"
      )
      exception
